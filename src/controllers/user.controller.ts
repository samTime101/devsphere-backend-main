...
export const createUser = async (req: Request, res: Response) => {
  try {
    ...
    const existingMember = await memberServices.getMember(memberId);

    if (existingMember.error) {
      return res.status(HTTP.BAD_REQUEST)
        .json(ErrorResponse(HTTP.BAD_REQUEST, existingMember.error))
    };
    ...
    if (existingUser.data?.id) {
      return res.status(HTTP.CONFLICT)
        .json(ErrorResponse(HTTP.CONFLICT, "User with provided email already exists"))
    }
