...
import { startContributorCron } from '@/jobs/contributor.cron';

const requiredEnvVars = ['CORS_ORIGIN', 'BETTER_AUTH_SECRET', 'BETTER_AUTH_URL'];
...
optionalEnvVars.forEach((env) => {
  ...
  console.info(
    `${env} IS ${process.env[env] ? 'CONFIGURED' : 'NOT CONFIGURED DEFAULTING TO EMPTY STRING'}`
  );
});
...
async function startServer() {
  try {
    ...
    // Start the contributor cron job
    if (NODE_ENV !== 'test') {
      startContributorCron();
    }
  } catch (error) {
