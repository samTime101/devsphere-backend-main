 ...
 * @update      2025-09-26 |
 *              - Refactored and image upload functionality
...
class EventService {
  async createEventService(
     ...
     * @param {Express.Multer.File[]} imageFiles - The image files to upload
     * @param {EventImageType[]} imageFileTypes - The types of the images
     ...
     * @example
     * const result = await createEventService(eventData, imageFiles, ["PROMOTIONAL", "GALLERY"]);
     */
    try {
  ...
  async getEventService(
     ...
     * @example
     * const result = await getEventService('uuid-of-event');
     */
    try {
      const [dbError, dbEvent] = await prismaSafe(
  ...
  async listEventService(): Promise<{
     ...
     * @throws Will throw an error if database operation fails.
     * @example
     * const result = await listEventService();
     */
    try {
  ...
  async updateEventService(
     ...
     * @throws Will throw an error if database operation fails.
     * @todo Handle image updates and dont delete existing schedules, just update them and add example for this method
     *
     */
    try {
  ...
  async deleteEventService(
     ...
     * @throws Will throw an error if database operation fails.
     * @example
     * const result = await deleteEventService('uuid-of-event');
     */
