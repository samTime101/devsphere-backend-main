...
const resolveEntity = (model: string): Models | false => {
  return (
    modelToEnumMap[model] ??
    modelToEnumMap[model.charAt(0).toUpperCase() + model.slice(1)] ??
    (modelToEnumMap[model.toUpperCase()] ?? false)
  );
};
    ...
    query: {
      $allModels: {
        async $allOperations({ model, operation, args, query }: QueryContext) {
          ...
          if (model === "AuditLogs" || !actionsToAudit.includes(op) || !resolveEntity(model)) {
            return query(args);
          }
